...
$root_path = $_SERVER['DOCUMENT_ROOT'] . "/assets/img/";
// $root_path = "/home/codd4763/public_html/triyatna_profile/assets/img/";
// $root_path = "F:/xampp/htdocs/Mypersonal/assets/img/";
$root_path2 = "/assets/img";

$protocol = stripos($_SERVER['SERVER_PROTOCOL'], 'https') === 0 ? 'https://' : 'http://';
$doma = $protocol . $_SERVER['SERVER_NAME'] . $root_path2;
...
defined('FM_ROOT_URL') || define('FM_ROOT_URL', 'https://' . $http_host . '/' . $root_url);
...
if (isset($_GET['upload'])) {
    fm_show_header($domain, $admin, $mysqli); // HEADER
...
?>
    <div class="row clearfix">
            ...
            <div class="card">
                <div class="body">
                    ...
                    <p class="break-word">
                        <?php
                        $protocol = stripos($_SERVER['SERVER_PROTOCOL'], 'https') === 0 ? 'https://' : 'http://';
                        $filep = fm_enc(fm_convert_win($file_path));
                        $count = strlen("$root_path");
                        $coun = substr($filep, $count);
                        $pathmed = $protocol . $_SERVER['SERVER_NAME'] . $root_path2 . $coun;
                        ?>

                        Full path: <input type="text" class="form-control" value="<?php echo fm_enc(fm_convert_win($file_path)) ?>">
                        Link : <br><input type="text" class="form-control" id='myInput' value="<?= $pathmed ?>">
                    <div class="container">
                        <div class="row">
                            <div class="col-sm">
                    ...
                    <p>
                        ...
                        <b class="copy-text"><a href="" onclick="Clipboard()"><i class="icon-copy"></i> Copy link</a></b>&nbsp;
                        <?php
                        // ZIP actions
                        if ($is_zip && $filenames !== false) {
        ...
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="body">
                    <p style="margin-bottom: 20px;"><strong>Preview</strong></p>
                    <?php
                    if ($is_zip) {
                        // ZIP content
                        if ($filenames !== false) {
                        ...
                        if (in_array($ext, array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico'))) {
                            echo '
                <div class="webpreview-phone docs-pictures">
                <p><img data-original="' . fm_enc($pathmed) . '" src="' . fm_enc($pathmed) . '" alt="" class="preview-img"></p>
                </div>
                ';
                        }
                    } elseif ($is_audio) {
                        // Audio content
                        echo '<p><audio src="' . fm_enc($pathmed) . '" controls preload="metadata"></audio></p>';
                    } elseif ($is_video) {
                        // Video content
                        echo '<div class="preview-video"><video src="' . fm_enc($pathmed) . '" width="640" height="360" controls preload="metadata"></video></div>';
                    } elseif ($is_text) {
...
$all_files_size = 0;
    ?>
    <form action="" method="post" class="filedata">
        <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
        ...
        <table>
            <tr>
                ...
                <?php if (!FM_IS_WIN) : ?><th style="width:6%">Perms</th>
            ...
            ?>
                <tr>
                    <td></td>
                    <td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode($parent) ?>"><i class="icon-arrow_up"></i> ..</a></td>
                </tr>
            <?php
            }
            foreach ($folders as $f) {
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <div class="filename"><a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div>
                    </td>
                    <td>Folder</td>
            ...
            ?>
                <tr>
                    <td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
                    <td>
                        <div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div>
                    </td>
        ...
        <p class="path"><a href="#" onclick="select_all();return false;"><i class="icon-checkbox"></i> Select all</a> &nbsp;
            ...
            <a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> Invert selection</a>
        </p>
        <p style="margin-left:20px"><input type="submit" name="delete" value="Delete" class="btn-danger" onclick="return confirm('Delete selected files and folders?')">
            <input type="submit" name="zip" value="Pack" onclick="return confirm('Create archive?')">
            <input type="submit" name="copy" value="Move">
        </p>
    </form>
    ...
    function fm_show_nav_path($path)
    {
        ...
        $total = $limit - folderSize($_SERVER['DOCUMENT_ROOT'] . "/assets/img/");
        function percent($limit, $total)
        {
            $perc = round($limit / $total * 100, 2);
            $percent = 100 - $perc;
            return $percent;
        }
    ...
    ?>
        <div class="row clearfix">
            <div class="col-lg-3 col-md-6 col-sm-12">
                <div class="card">
                    <div class="body l-green">
                        <h4 class="m-t-0"><?= getSymbolByQuantity(disk_free_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/")); ?> <i class="zmdi zmdi-storage"></i></h4>
                        <div class="progress m-t-10">
                            <div class="progress-bar l-dark" role="progressbar" aria-valuenow="<?= percent(disk_free_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/"), disk_total_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/")) ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?= percent(disk_free_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/"), disk_total_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/")) ?>%;"></div>
                        </div>
                        <small class="info text-muted">Storage of <?= getSymbolByQuantity(disk_total_space($_SERVER['DOCUMENT_ROOT'] . "/assets/img/")); ?></small>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-12">
                <div class="card">
                    <div class="body">
                        <h4 class="m-t-0"><?= getSymbolByQuantity($total); ?> <i class="zmdi zmdi-collection-folder-image"></i></h4>
                        <div class="progress m-t-10">
                            <div class="progress-bar l-blue" role="progressbar" aria-valuenow="<?= percent($total, $limit) ?>" aria-valuemin="0" aria-valuemax="100" style="width:<?= percent($total, $limit) ?>%;"></div>
                        </div>
                        <small class="info text-muted">Media of <?= getSymbolByQuantity($limit); ?></small>
                    </div>
                </div>
            </div>
        </div>
        <div class="path filedata">
            ...
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='?p='><i class='zmdi zmdi-home' style='font-weight: bold;' title='" . FM_ROOT_PATH . "'></i></a><a href='javascript:history.back();' title='Undo'> <i class='zmdi zmdi-arrow-left ml-3' style='font-weight: bold;'></i></a><a href='javascript:history.forward();' title='Redo'> <i class='zmdi zmdi-arrow-right ml-3' style='font-weight: bold;'></i></a><a href='javascript:location.reload();' title='Refresh'> <i class='zmdi zmdi-replay ml-3' style='font-weight: bold;'></i></a>";
     ...
     * Show page header
     */
    function fm_show_header($domain, $admin, $mysqli)
    {
    ...
    ?>
        ...
        <head>
            ...
            <meta name="description" content="Panel Admin for control Personal portfolio by">
            <title><?= getSingleValDB('options', 'id', '1', 'name')  ?> - File Manager</title>
            ...
            <link rel="stylesheet" href="<?= $admin ?>assets/plugins/viewerjs/dist/viewer.css">
    ...
    <script>
        document.getElementById("filemanage").classList.add('active');
    </script>
    <script>
    ...
    <script src="<?= $admin ?>assets/plugins/viewerjs/dist/viewer.js"></script>
